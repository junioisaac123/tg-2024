...
const $btnRestartGame = $gameOverBanner.querySelector('[data-btn-action="restart-game"]');
...
const $continueGameSection = $modalGame.querySelector("[data-state='resume-game']");

const $btnStartNewGame = $modalGame.querySelector("[data-btn-action='start-new-game']");
const $btnContinueGame = $modalGame.querySelector("[data-btn-action='continue-game']");
const $btnResetNewGame = $modalGame.querySelector("[data-btn-action='reset-new-game']");
...
function removeGreySquares() {
    ...
    $$(`${BOARD_DOM_ID} .square-55d63`).forEach(el => el.style.background = '')
...
function updateStatus() {
    ...
    if (gameState.engine.in_checkmate()) {
        const nextColor = gameState.engine.turn() === 'w' ? 'Negras' : 'Blancas'; 
...
function startNewGame() {
    const selectedColor = $modalGame.querySelector('input[name="color"]:checked');
    if (!selectedColor) {
        return;
    }
    const color = selectedColor.value == 'random' ? (Math.random() >= 0.5 ? "black" : "white") : selectedColor.value;
    instanceBoard(color);
    setState();
    hiddenModal();
}
...
$btnResetNewGame.addEventListener("click", function () {
    confirm("¿Estás seguro de que quieres reiniciar el juego?") && (function () {
        hiddenModal();
        showNewGameModal()
    })();
});
